...
parser.add_argument('--split_all', type=int, nargs='?', default=50, help='split_all')
...
if '_cs' in FLAGS.datadir:
    """
    CSWin based model
    """
    adj_all, adj_att, features, y_train_s, y_val_s, idx_train_s, idx_val_s, train_mask_s, test_mask_s, lookup_table = \
        load_data_action_zero_shot_all(FLAGS.dataset, FLAGS.w2v_type, FLAGS.split_ind, data_path = data_path)
    label_num = len(train_mask_s[0])
    ...
    for SPLIT_IND in range(0,FLAGS.split_all):
        print("Split: "+str(SPLIT_IND))
        if FLAGS.model == 'dense':
            model_func = GCN_dense_mse_2s

        train_mask=train_mask_s[SPLIT_IND]
        test_mask=test_mask_s[SPLIT_IND]
        ...
        y_train = np.array(y_train_s[SPLIT_IND])
        idx_train = np.array(idx_train_s[SPLIT_IND])
        y_val = np.array(y_val_s[SPLIT_IND])
        idx_val = np.array(idx_val_s[SPLIT_IND])
        ...
        if FLAGS.setting == "generalized_transductive":         
            f=open(data_path+'/split_random_id','rb')
            alldata=pkl.load(f)
            y_train =alldata[SPLIT_IND][0]
            idx_train =alldata[SPLIT_IND][1]
            y_val =alldata[SPLIT_IND][2]
            idx_val = alldata[SPLIT_IND][3]
        ...
        for epoch in range(FLAGS.epochs):
            if not FLAGS.mode == "INIT":
                model.train()
                count = 0
                rand_inds = np.random.permutation(train_num)      
                rand_inds = rand_inds[:int(len(rand_inds)/FLAGS.batch_size)*FLAGS.batch_size]
                rand_inds = np.reshape(rand_inds,[-1, FLAGS.batch_size])
                tdy_train=np.concatenate((y_train,y_val))
                tdidx_train=np.concatenate((idx_train,idx_val))

                for inds in rand_inds[:int(len(rand_inds))]:
                    # Construct feed dictionary
                    optimizer.zero_grad()
                    label = tdy_train[inds]
                    video_idx = tdidx_train[inds]
                    ...
                    if count % 1 == 0:
                        ...
                        count += 1
            test_accuracy = 0
            test_inds = np.random.permutation(test_num)
            print('test video len is :{}'.format(len(test_inds)))
            test_inds = test_inds[:int(len(test_inds) / 8) * 8]
            test_inds = np.reshape(test_inds, [-1, int(8*1)])
            count_test = 0
            model.eval()
            with torch.no_grad():
